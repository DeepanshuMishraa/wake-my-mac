...
const comparisonLinks = ["Amphetamine", "Caffeine", "KeepingYouAwake"];
...
export default function Home() {
  ...
  const jsonLd = {
    ...
    "@graph": [
      {
        "@type": "Organization",
        "@id": "https://wakemymac.com/#organization",
        name: "Wake My Mac",
        url: "https://wakemymac.com/",
        sameAs: ["https://github.com/DeepanshuMishraa/wake-my-mac"],
        contactPoint: { "@type": "ContactPoint", email: "hello@wakemymac.com", contactType: "customer support" },
      },
      {
        "@type": "WebSite",
        "@id": "https://wakemymac.com/#website",
        url: "https://wakemymac.com/",
        name: "Wake My Mac",
        description: "A free, private macOS utility that keeps your Mac awake while long-running work finishes.",
        inLanguage: "en-US",
      },
      {
        "@type": "SoftwareApplication",
        "@id": "https://wakemymac.com/#software",
        name: "Wake My Mac",
        ...
        featureList: ["Keep Mac awake", "Closed-lid support", "Battery cutoff", "SSH and remote-work support", "Local activity history"],
        offers: { "@type": "Offer", price: "0", priceCurrency: "USD", availability: "https://schema.org/InStock" },
        image: "https://wakemymac.com/dashboard-preview.png",
        publisher: { "@id": "https://wakemymac.com/#organization" },
        isPartOf: { "@id": "https://wakemymac.com/#website" },
      },
      {
        "@type": "FAQPage",
        "@id": "https://wakemymac.com/#faq",
  ...
  return <main>
    ...
    <Image src="/dashboard-preview.png" alt="Wake My Mac dashboard showing a Mac staying awake for active work" className="product-image shell" width={2428} height={1762} priority />
    ...
    <section className="comparison shell" aria-labelledby="comparison-heading">
      <div>
        <p className="eyebrow">Looking for a Mac caffeine app?</p>
        <h2 id="comparison-heading">A calmer way to<br /><em>hold your Mac awake.</em></h2>
      </div>
      <div className="comparison-copy">
        <p>If you searched for “hold my Mac awake,” an Amphetamine alternative, a Caffeine app, or KeepingYouAwake, Wake My Mac is built for the same core job: prevent sleep while the work you started is still running.</p>
        <p>It keeps the useful parts simple—closed-lid support, battery guardrails, and local controls—so a long download, coffee break, build, render, or SSH session can finish without a cloud account.</p>
        <div className="comparison-terms" aria-label="Related Mac keep-awake searches">{comparisonLinks.map((name) => <span key={name}>{name} alternative</span>)}</div>
      </div>
    </section>

    <section id="faqs" className="faq-section shell"><div className="faq-heading"><p className="eyebrow">Questions, answered</p><h2>Good to<br /><em>know.</em></h2><p>Still unsure? <a href="mailto:hello@wakemymac.com">Ask us directly ↗</a></p></div><div className="faq-list">{faqs.map(([question, answer], index) => <div className={`faq ${activeFaq === index ? "open" : ""}`} key={question}><button id={`faq-question-${index}`} onClick={() => setActiveFaq(activeFaq === index ? null : index)} aria-expanded={activeFaq === index} aria-controls={`faq-answer-${index}`}><span>{question}</span><span className="plus" aria-hidden="true">+</span></button><div id={`faq-answer-${index}`} className="faq-answer" role="region" aria-labelledby={`faq-question-${index}`}><p>{answer}</p></div></div>)}</div></section>
